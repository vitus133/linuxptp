		...
		for (i = 0; i < n_domains; i++) {
			...
			LIST_FOREACH(clock, &domain->dst_clocks, dst_list) {
				...
				if (clock->clkid == domain->master->clkid ||
				    (clock->phc_index >= 0 &&
				     clock->phc_index == domain->master->phc_index) ||
				    !strcmp(clock->device, domain->master->device))
					continue;

				if (clock->clkid == CLOCK_REALTIME &&
				    domain->master->sysoff_method >= 0) {
					/* use sysoff */
					err = sysoff_measure(CLOCKID_TO_FD(domain->master->clkid),
							     domain->master->sysoff_method,
							     domain->phc_readings,
							     &offset, &ts, &delay);
				} else if (domain->master->clkid == CLOCK_REALTIME &&
					...
					/* use phc */
					err = clockadj_compare(domain->master->clkid,
							       clock->clkid,
							       domain->phc_readings,
							       &offset, &ts, &delay);
				}
...
int main(int argc, char *argv[])
{
	...
	int i, j, autocfg = 0, c, domain_number = 0, index, ntpshm_segment, offset;
	...
	for (i = 0; i < n_domains; i++) {
		domains[i] = settings;
		domains[i].free_running = config_get_int(cfg, NULL, "free_running");
		domains[i].servo_type = config_get_int(cfg, NULL, "clock_servo");
		if (domains[i].free_running || domains[i].servo_type == CLOCK_SERVO_NTPSHM) {
			config_set_int(cfg, "kernel_leap", 0);
			config_set_int(cfg, "sanity_freq_limit", 0);
		}
		domains[i].kernel_leap = config_get_int(cfg, NULL, "kernel_leap");
		domains[i].sanity_freq_limit = config_get_int(cfg, NULL, "sanity_freq_limit");
	...
	if (wait_sync) {
		for (i = 0; i < n_domains; i++) {
			snprintf(uds_local, sizeof(uds_local), "/var/run/phc2sys.%d.%d",
				 getpid(), i);
